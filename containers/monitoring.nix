...
in
{
  ...
  config = lib.mkIf (cfg.grafana.enable || cfg.prometheus.enable || cfg.node-exporter.enable || cfg.cadvisor.enable) {
    ...
    virtualisation.arion.projects.monitoring.settings = {
      ...
      networks.exporter.internal = lib.mkIf (cfg.prometheus.enable && (cfg.cadvisor.enable || cfg.node-exporter.enable)) true;
      ...
      grafana.service =  {
        ...
        prometheus.service = {
          ...
          user = "0:0";
          volumes = [
            "${config.lib.server.mkConfigDir "prometheus"}/prometheus.yaml:/etc/prometheus/prometheus.yaml:ro"
