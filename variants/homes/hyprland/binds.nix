...
in
{
  wayland.windowManager.hyprland.settings.bind = [
    ...
    (mkBind "SUPER + V" (
      exec "killall rofi || cliphist list | rofi -dmenu -display-columns 2 -theme .config/rofi/themes/rofi-dmenu.rasi | cliphist decode | wl-copy"
    ))
    # Lock
    (mkBind "SUPER + escape" (
      exec "killall wlogout || wlogout --protocol layer-shell -b 5 -T 400 -B 400"
    ))
    # Screenshot
    (mkBind "SUPER + SHIFT + S" (
      exec ''hyprctl keyword "animation,fadeOut,0,0,default"; grimblast --notify copysave area; hyprctl keyword "animation,fadeOut,1,4,default"''
    ))
    # Window management
  ...
  ++ [
    ...
    (mkBindOpts "XF86AudioRaiseVolume" (exec "wpctl set-volume @DEFAULT_AUDIO_SINK@ 5%+") {
      repeating = true;
    })
    (mkBindOpts "XF86AudioLowerVolume" (exec "wpctl set-volume @DEFAULT_AUDIO_SINK@ 5%-") {
      repeating = true;
    })
    (mkBindOpts "XF86AudioMute" (exec "wpctl set-mute @DEFAULT_AUDIO_SINK@ toggle") {
      repeating = true;
    })
    (mkBindOpts "XF86AudioMicMute" (exec "wpctl set-mute @DEFAULT_AUDIO_SOURCE@ toggle") {
      repeating = true;
    })
    ...
    # Resize (repeating)
    (mkBindOpts "SUPER + CTRL + left" "hl.dsp.window.resize({ x = -20, y = 0, relative = true })" {
      repeating = true;
    })
    (mkBindOpts "SUPER + CTRL + right" "hl.dsp.window.resize({ x = 20, y = 0, relative = true })" {
      repeating = true;
    })
    (mkBindOpts "SUPER + CTRL + up" "hl.dsp.window.resize({ x = 0, y = -20, relative = true })" {
      repeating = true;
    })
    (mkBindOpts "SUPER + CTRL + down" "hl.dsp.window.resize({ x = 0, y = 20, relative = true })" {
      repeating = true;
    })
    # Mouse drag binds
